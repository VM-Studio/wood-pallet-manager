...
exports.getEstadisticasDevolucionesService = exports.cancelarDevolucionService = exports.confirmarDepositoService = exports.crearDevolucionService = exports.getDevolucionByIdService = exports.getDevolucionesService = void 0;
...
// ── GET ALL ─────────────────────────────────────────────────────────────────
const getDevolucionesService = async (usuarioId) => {
    return prisma_1.default.devolucion.findMany({
        orderBy: { fechaSolicitud: 'desc' },
        include: devolucionInclude,
    });
};
exports.getDevolucionesService = getDevolucionesService;
// ── GET BY ID ────────────────────────────────────────────────────────────────
const getDevolucionByIdService = async (id) => {
    return prisma_1.default.devolucion.findUnique({
        where: { id },
        include: devolucionInclude,
    });
};
exports.getDevolucionByIdService = getDevolucionByIdService;
// ── CREATE ───────────────────────────────────────────────────────────────────
const crearDevolucionService = async (datos, usuarioId) => {
    ...
    if (datos.devuelveSenasa) {
        const cotizacion = venta.cotizacionId
            ? await prisma_1.default.cotizacion.findUnique({ where: { id: venta.cotizacionId } })
            : null;
    ...
    const requiereConfirmacion = datos.tipoCaso === 'cliente_no_quiere' ||
    ...
    const devolucion = await prisma_1.default.$transaction(async (tx) => {
...
exports.crearDevolucionService = crearDevolucionService;
// ── CONFIRMAR DEPOSITO ────────────────────────────────────────────────────────
const confirmarDepositoService = async (id, usuarioId) => {
    const devolucion = await prisma_1.default.devolucion.findUnique({
        where: { id },
        include: {
            venta: {
                include: {
    ...
    if (devolucion.estado === 'cancelada')
        throw new Error('La devolución está cancelada');
    return prisma_1.default.$transaction(async (tx) => {
        ...
        if (!devolucion.compensaEnSiguientePedido) {
            await procesarDevolucionInterna(tx, devolucion, devolucion.venta, usuarioId);
        }
        else {
            await tx.devolucion.update({
                where: { id },
...
exports.confirmarDepositoService = confirmarDepositoService;
// ── PROCESAR DEVOLUCIÓN (restaurar stock + actualizar factura) ────────────────
async function procesarDevolucionInterna(tx, devolucion, venta, usuarioId) {
...
// ── CANCELAR ──────────────────────────────────────────────────────────────────
const cancelarDevolucionService = async (id) => {
    const dev = await prisma_1.default.devolucion.findUnique({ where: { id } });
    if (!dev)
        throw new Error('Devolución no encontrada');
    if (dev.estado === 'procesada')
        throw new Error('No se puede cancelar una devolución ya procesada');
...
exports.cancelarDevolucionService = cancelarDevolucionService;
// ── STATS para reportes ───────────────────────────────────────────────────────
const getEstadisticasDevolucionesService = async () => {
    const [total, porCaso, montoTotal] = await Promise.all([
        prisma_1.default.devolucion.count({ where: { estado: { not: 'cancelada' } } }),
        prisma_1.default.devolucion.groupBy({
            by: ['tipoCaso'],
            _count: { id: true },
            where: { estado: { not: 'cancelada' } },
        }),
        prisma_1.default.devolucion.aggregate({
...
exports.getEstadisticasDevolucionesService = getEstadisticasDevolucionesService;
