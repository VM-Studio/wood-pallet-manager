...
export async function crearSolicitud(req: Request, res: Response) {
  try {
    ...
    const solicitud = await crearSolicitudService(usuario.id, req.body);
...
export async function getSolicitudes(req: Request, res: Response) {
  try {
    const usuario = (req as any).user;
    const solicitudes = await getSolicitudesService(usuario.id, usuario.rol);
...
export async function responderSolicitud(req: Request, res: Response) {
  try {
    ...
    const solicitud = await responderSolicitudService(id, usuario.id, estado, notasRespuesta);
    res.json(solicitud);
  } catch (error: any) {
    res.status(400).json({ error: error.message });
  }
